...
import { LayoutGrid, Map, MessageCircle, User, ShoppingBag } from 'lucide-react';

const NAV_ITEMS = [
  { id: 'booking', Icon: LayoutGrid,   getLbl: t => t.navBooking },
  { id: 'map',     Icon: Map,           getLbl: t => t.navMap     },
  { id: 'market',  Icon: ShoppingBag,   getLbl: () => 'Маркет'    },
  { id: 'chat',    Icon: MessageCircle, getLbl: t => t.navChat    },
  { id: 'profile', Icon: User,          getLbl: t => t.navProfile },
];

const PAGE_TITLES = {
  booking: t => t.navBooking,
  map:     t => t.navMap,
  market:  () => '🏪 Маркет снаряжения',
  chat:    t => t.navChat,
  profile: t => t.navProfile,
};
...
export default function TouristShell() {
  ...
  // Hide bottom nav when inside a chat room (so input isn't blocked)
  const [navHidden,  setNavHidden]  = useState(false);

  // Handle OPEN_CHAT dispatched by BookingTab
  useEffect(() => {
  ...
  const switchTab = (id) => {
    setActiveTab(id);
    // If leaving chat tab, ensure nav reappears
  ...
  return (
    <div className="tap-screen relative overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 pt-safe pt-4 pb-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="text-text-muted text-xs ml-1.5">Жетысу</span>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <AnimatePresence>
            {langOpen && (
              <motion.div
                className="absolute right-0 top-9 bg-card-mid border border-white/10 rounded-2xl
      ...
      {/* ── Stories strip (booking tab only) ─────── */}
      {activeTab === 'booking' && (
      ...
      {/* ── Bottom Nav (hidden when chat room is open) ── */}
      <AnimatePresence>
        {!navHidden && (
          <motion.nav
              ...
              return (
                <motion.button key={id} onClick={() => switchTab(id)}
                  ...
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all
                    ${active ? 'bg-blazing-orange' : 'bg-transparent'}`}>
                    <Icon size={19} className={active ? 'text-white' : 'text-text-muted'} />
                  </div>
                  {badge && (
                    <div className="absolute top-0.5 right-2 min-w-[16px] h-4 rounded-full bg-red-500
